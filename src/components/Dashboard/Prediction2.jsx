...
function Prediction2() {
    ...
    function handle() {
        ...
        console.log(crop);
        fetch(
            `https://coperiax-server2.onrender.com/predict?temperature=${propsData.temperature}&humidity=${propsData.humidity}&moisture=${propsData.moisture}&soil=${soil}&crop=${crop}&N=${propsData.N}&P=${propsData.P}&K=${propsData.K}`
        )
            .then((res) => res.json())
            .then((data) => {
        ...
        <div className="bg-[#dde7c7]">
            ...
            <div className="flex justify-center w-full md:block md:w-10/12 md:m-auto p-2 ml-4">
                {state ? (
                        ...
                        <div className=" bg-green-200 w-11/12 rounded-2xl h-fit p-20 md:grid md:grid-cols-2 gap-4">
                            <div>
                                <div className="text-green-700 font-bold">
                                    Nitrogen Content : {propsData.N}
                                </div>
                            </div>
                            <div>
                                <div className="text-green-700 font-bold">
                                    Phosphorous Content : {propsData.P}
                                </div>
                            </div>
                            <div>
                                <div className="text-green-700 font-bold">
                                    Potassium Content : {propsData.K}
                                </div>
                            </div>
                            <div>
                                <div className="text-green-700 font-bold">
                                    Temperature :{propsData.temperature}
                                </div>
                            </div>
                            <div>
                                <div className="text-green-700 font-bold">
                                    Humidity : {propsData.humidity}
                                </div>
                            </div>
                            <div>
                            ...
                            <button
                                ...
                                className=" hover:bg-green-400 md:block m-auto h-fit w-full first-letter: bg-green-600 p-2 mt-2 rounded-full"
                            >
                                Predict
                            </button>
                            <a href="/ins">
                    ...
                    <div className=" w-full bg-slate-50 md:flex md:justify-center text-green-700 text-2xl h-fit p-10">
                        Fertilizer Suitable Recommended:
                        <span className="font-bold text-3xl p-2 block ">
                            {fert}
                        </span>
                        <Link className="link" to="/dashboard">
                            <div className="hover:bg-green-400 md:block text-lg  text-black h-fit w-fit bg-green-600 md:pl-4 ml-4  p-2 rounded-full">
                                Return To Dashboard
                            </div>
                        </Link>
                    </div>
